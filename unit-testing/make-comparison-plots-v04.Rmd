...
Total Distance = $\frac{1}{3}$ ( log(expense distance ) + geo distance + mission distance) 

The red line is the value for the comparison org that has rank 1000 in total distance. 

### These are the plots for the unnormalized distances 
...
# plot 1 
dataLine <- dat %>%
  select(dist, Rank, Weights.Set, Test.Org) %>%
  group_by(Weights.Set, Test.Org) %>%
  filter(Rank == 1000)
...
dat %>%
  ...
  ggplot(aes(x = dist)) + 
  geom_density()+
  facet_grid(as.factor(Test.Org) ~ Weights.Set, scales = "free") +
  geom_vline(data = dataLine, aes(xintercept = dist), color = "red") +
  ggtitle("Total Distace - Unnormalized") +
  theme(strip.text.y = element_text(size = 5))+
  xlab("total distance ")  

#plot 2
dataLine <- dat %>%
  select(dist.log.expense, Rank, Weights.Set, Test.Org) %>%
  group_by(Weights.Set, Test.Org) %>%
  filter(Rank == 1000)
...
dat %>%
  ...
  geom_density()+
  facet_grid(as.factor(Test.Org) ~ Weights.Set, scales = "free") + 
  geom_vline(data = dataLine, aes(xintercept = dist.log.expense), color = "red") +
  ggtitle("log(Expense) Distance  - Unnormalized") +
  theme(strip.text.y = element_text(size = 5)) +
...
# Plot 3
dataLine <- dat %>%
  select(dist.geo, Rank, Weights.Set, Test.Org) %>%
  group_by(Weights.Set, Test.Org) %>%
  filter(Rank == 1000)
...
dat %>%
  ...
  geom_density()+
  facet_grid(as.factor(Test.Org) ~ Weights.Set, scales = "free") + 
  geom_vline(data = dataLine, aes(xintercept = dist.geo), color = "red") +
  ggtitle("Geographic Distace  - Unnormalized") +
  theme(strip.text.y = element_text(size = 5))+
...
#Plot 4
dataLine <- dat %>%
  select(dist.mission, Rank, Weights.Set, Test.Org) %>%
  group_by(Weights.Set, Test.Org) %>%
  filter(Rank == 1000)
...
dat %>%
  ...
  geom_density()+
  facet_grid(as.factor(Test.Org) ~ Weights.Set, scales = "free") + 
  geom_vline(data = dataLine, aes(xintercept = dist.mission), color = "red") +
  ggtitle("Mission Distace  - Unnormalized") +
  theme(strip.text.y = element_text(size = 5))+
...
### These are the plots for the normalized distances 


```{r plots.norm, echo=FALSE}
# plot 1 
dataLine <- dat %>%
  select(dist.norm, Rank, Weights.Set, Test.Org) %>%
  group_by(Weights.Set, Test.Org) %>%
  filter(Rank == 1000)
  
dat %>%
  select(dist.norm, Weights.Set, Test.Org) %>%
  ggplot(aes(x = dist.norm)) + 
  geom_density()+
  facet_grid(as.factor(Test.Org) ~ Weights.Set, scales = "free") +
  geom_vline(data = dataLine, aes(xintercept = dist.norm), color = "red") +
  ggtitle("Total Distace - Normalized") +
  theme(strip.text.y = element_text(size = 5))+
  xlab("total distance ")  

#plot 2
dataLine <- dat %>%
  select(dist.log.expense.norm, Rank, Weights.Set, Test.Org) %>%
  group_by(Weights.Set, Test.Org) %>%
  filter(Rank == 1000)

dat %>%
  select(dist.log.expense.norm, Weights.Set, Test.Org) %>%
  ggplot(aes(x = dist.log.expense.norm)) + 
  geom_density()+
  facet_grid(as.factor(Test.Org) ~ Weights.Set, scales = "free") + 
  geom_vline(data = dataLine, aes(xintercept = dist.log.expense.norm), color = "red") +
  ggtitle("log(Expense) Distance  - Normalized") +
  theme(strip.text.y = element_text(size = 5)) +
  xlab("log(Expense) distance")
...
# Plot 3
dataLine <- dat %>%
  select(dist.geo.norm, Rank, Weights.Set, Test.Org) %>%
  group_by(Weights.Set, Test.Org) %>%
  filter(Rank == 1000)

dat %>%
  select(dist.geo.norm, Weights.Set, Test.Org) %>%
  ggplot(aes(x = dist.geo.norm)) + 
  geom_density()+
  facet_grid(as.factor(Test.Org) ~ Weights.Set, scales = "free") + 
  geom_vline(data = dataLine, aes(xintercept = dist.geo.norm), color = "red") +
  ggtitle("Geographic Distace  - Normalized") +
  theme(strip.text.y = element_text(size = 5))+
  xlab("geographic distance ")
...
#Plot 4
dataLine <- dat %>%
  select(dist.mission.norm, Rank, Weights.Set, Test.Org) %>%
  group_by(Weights.Set, Test.Org) %>%
  filter(Rank == 1000)

dat %>%
  select(dist.mission.norm, Weights.Set, Test.Org) %>%
  ggplot(aes(x = dist.mission.norm)) + 
  geom_density()+
  facet_grid(as.factor(Test.Org) ~ Weights.Set, scales = "free") + 
  geom_vline(data = dataLine, aes(xintercept = dist.mission.norm), color = "red") +
  ggtitle("Mission Distace  - Normalized") +
  theme(strip.text.y = element_text(size = 5))+
  xlab("mission distance ")
...
These plots display the unnormalized distance
