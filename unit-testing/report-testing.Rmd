...
results %>%
  ...
  geom_vline(xintercept = weighted.average, col = "green") +
  geom_vline(xintercept = suggested.range[1], col = "blue") +
  ...
  labs(title = "Actual vs. Suggested CEO Pay",
              ...
              caption = "red = actual pay, green = weighted average, green = suggested pay range")+
  xlab("CEO Pay") +
  ylab(" ")
...
D.percent <- matrix(0, ncol = dim(distance.matrix)[2], nrow = dim(distance.matrix)[1])
for(i in 1:( dim(distance.matrix)[1])){
  for(j in 1:(dim(distance.matrix)[2])){
    D.percent[i,j] <- distance.matrix[i, j] / dists[i]
  }
}

#D.percent.average <- colSums(D.percent) / dim(distance.matrix)[1]
names(D.percent.average) <- names(D.average)


D.corr <- rep(0, 9)
for(i in 1:9){
  D.corr[i] <- cor(distance.matrix[, i], dists)
}


D.tab <- data.frame(Attribute = names(D.average),
                    Average = D.average, 
                    Percent = D.percent.average,
                    Correlation = D.corr)
...
cbind(distance.matrix, dists) %>%
  select(-c(MajorGroup, NTEE, NTEE.CC, UNIV, HOSP, Loc)) %>%
  plot(main = "Correlation of Distances")
