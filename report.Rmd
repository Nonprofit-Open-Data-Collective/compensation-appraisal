---
title: "Urban Institute's CEO Compensation Appasial Tool Results"
output: pdf_document
params:
  dat.final: NA
  org: NA
...
#get parameters 
dat.final <- params$dat.final
org <- params$org
search <- params$search
hard <- params$hard

#get results 
results <- HEOM_with_weights(org, search, dat.final)
...
tab.major.group <- table(dat.final$MajorGroup)
tab.ntee <- table(dat.final$NTEE)
...
ggplot(data = dat.final) +
  ...
  labs(caption = paste("Note, there were", sum(dat.final$CEOCompensation > 100000) , "nonprofits who paid there CEO more than $1,000,000 that are not displayed on this plot."))
