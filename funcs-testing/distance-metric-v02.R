HEOM_with_weights <- function(org, search, dat.filtered, weights){
  ...
  for(i in 1:n){
    ...
    if(org.mission$type.org != temp.org$type.org){
      ...
      cols <- c("broad.category", "letter", "letter1", "letter2")
      ...
      temp.comp <- data.frame(
                     ...
                     s.further = ifelse(is.na(temp.org$s.further), 100, temp.org$s.further ))
      org.comp <- data.frame(
                       ...
                       s.further = ifelse(is.na(org.mission$s.further), 101, org.mission$s.further ))             
      D[i, "Mission"] <- sum( s.mission.weights$weight * (temp.comp != org.comp ))                
    }
...
# 
# org <- orgs.list[[3]]
# search <- search.list[[3]]
# hard <- hard.list[[3]]
# dat.filtered <- dat_filtering_hard(search, hard, 350868101)
# 
# weights <- list(geo.weight = data.frame(level = c(1, 2, 3),
#                                         weight = c(6, 6, 1) / 24 ), #max(state_dist) = 12)
#                 r.mission.weights = data.frame(level = c(1, 2, 3, 4, 5), 
