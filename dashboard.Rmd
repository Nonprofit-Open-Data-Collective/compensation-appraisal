...
#install packages + load libraries libraries 
list.of.packages <- c("flexdashboard", "shiny", "ggplot2", "dplyr", "kableExtra", 
                      "DT", "shinyWidgets", "plotly", "devtools")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)>0){ install.packages(new.packages)}

#install compensator
if((!"compensator" %in%installed.packages()[,"Package"]) ){
  devtools::install_github("Nonprofit-Open-Data-Collective/compensator")
}

lapply(c(list.of.packages, "compensator"), require, character.only = TRUE) 

ntee.codes <- read.csv("data-raw/ntee-crosswalk.csv")
ntee.values <- read.csv("data-raw/ntee-original.csv")

data("ntee.orig", package = "compensator")
data("ntee.crosswalk", package = "compensator")
...
Filters selected on this page will not impact the appraisal process. 
...
dat.gender.graph <- reactive({
  ...
  #yaxis options: "broad.category", 
    ...
    #add hosp and univ as broad categories - needed for data table
    dplyr::mutate(
      braod.category = case_when(
        univ == TRUE ~ "UNI",
        hosp == TRUE ~ "HOS",
        TRUE ~ broad.category
      )) %>%
    #add hosp and univ as major groups - needed for data table
    dplyr::mutate(
    ...
    dat.plot <- 
      ...
      #group by everything execept ceo.compensation
    ...
    if(y.axis == "broad.category"){
      dat.plot <- dat.plot %>%
        dplyr::mutate(Yaxis = case_when(
          ...
          Yaxis == "EDU" ~ "Education, Non-University",
          Yaxis == "ENV" ~ "Environment and Animals",
          Yaxis == "HEL" ~ "Health, Non-Hospital",
    ...
    if(y.axis == "major.group"){
      dat.plot <- dat.plot %>%
        dplyr::mutate(Yaxis = case_when(
          ...
          Yaxis == "B" ~ "Education, Non-Univeristy", 
          Yaxis == "C" ~ "Environment",
          Yaxis == "D" ~ "Animal-Related",
          Yaxis == "E" ~ "Health Care, Non-Hospital",
...
renderPlotly({
  ...
  #formatted data + correct summary statistic
  dat.plot <- dat.gender.graph()%>%
...
DT::renderDataTable(
  DT::datatable({ 
    ...
    #If y.axis == major.group, only display the selected major groups 
    if(y.axis == "major.group"){
      #get the name of the selected major  groups
      letter.keep <- 
        ntee.crosswalk %>%
          dplyr::filter(broad.category %in% input$filter.gender.broad.category)%>%
          dplyr::select(major.group) %>%
          unique %>%
          dplyr::pull()
      ...
      dat.plot <- dat.plot %>%
        dplyr::filter(Yaxis %in% mg.keep)
    }
    
    #get summary statistics to display
    dat.plot <- 
      dat.plot%>%
      dplyr::summarise(
