...
```{r}
### Regular org
output$IsRegular <- reactive({
   input$ntee.rs == "regular"
})

outputOptions(output, "IsRegular", suspendWhenHidden = FALSE)

conditionalPanel(
  condition = "output.IsRegular",
  shinyWidgets::pickerInput(
   inputId = "ntee.broad.category",
   label = "Which of the following broad cateogires describes your nonprofit best?",
...
```{r}
# 
# pickerInput(inputId = "ntee.major.group",
#                label = "Major Group",
#             choices = "Arts, Culture & Humanities")
# 
#     
# observeEvent(input$ntee.major.group, {
#     which.letter <- ntee.codes %>% 
#       dplyr::filter(BroadCategory == as.numeric(input$ntee.broad.category)) %>%
#       select(MajorGroup) %>%
#       unique()
#   
...
#     
#     # Method 1
#     updatePickerInput(session = session, inputId = "ntee.major.group",
#                       choices = input$ntee.broad.category)
#     
# 
#   }, ignoreInit = TRUE)
