...
```{r}
##regular/specialty
selectInput("detailed.search.rs",
            "Do you want to include regular or specality organization in your search?", 
...
observe({
  ...
  which.letters <- ntee.codes %>%
      dplyr::filter(BroadCategory %in% as.numeric(bc)) %>%
      select(MajorGroup) %>%
      unique()

  which.mg <- ntee.values %>%
    filter(nchar(ntee) == 1) %>%
    filter(ntee %in% which.letters$MajorGroup)  %>%
    select(description) 
...
detailed.org <- eventReactive(input$detailed.update.comparison, {
   ...
   loc <- dplyr::case_when(input$detailed.org.location == "Metropolitan" ~ "metro",
                          input$detailed.org.location == "Rural" ~ "rural")
  
  compensator::get_org_values(state = input$detailed.org.state,
                                     location.type = loc,
...
detailed.search.criteria <- eventReactive(input$detailed.update.comparison,{
  ...
  #format search criteria
   ...
   list(broad.category = input$detailed.search.bc,
       ...
       univ = input$detailed.search.univ,
       hosp = input$detailed.search.hosp,
       location.type = ifelse(length(input$detailed.search.location) == 2,
                              "both",
                              input$detailed.search.location),
...
# renderText({
#   paste(detailed.search.criteria())
# })
...
NTEE <- compensator::ntee.crosswalk %>%
  ...
  mutate(description.tens = ifelse(is.na(description), "Specality", description)) %>% 
  left_join(dat.letter2, by = "letter2" ) %>%
  mutate(description.ones = description.y) %>%
