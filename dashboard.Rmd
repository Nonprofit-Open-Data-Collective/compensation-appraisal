...
search.criteria <- reactive( {
  ...
  #return
  list(
    broad.category = ifelse(
      org()$type.org == "regular", 
      org()$broad.category, 
      NA), 
...
observe({
  ...
  which.letters <- ntee.codes %>%
      ...
      dplyr::select(MajorGroup) %>%
      unique()

  which.mg <- ntee.values %>%
    dplyr::filter(nchar(ntee) == 1) %>%
    dplyr::filter(ntee %in% which.letters$MajorGroup)  %>%
...
detailed.org <- eventReactive(input$detailed.update.comparison, {
   ...
   loc <- 
     dplyr::case_when(
       input$detailed.org.location == "Metropolitan" ~ "metro",
...
detailed.search.criteria <- eventReactive(input$detailed.update.comparison,{
  ...
  search.letters <- ntee.values %>%
    ...
    dplyr::filter(description %in% mg) %>% 
    pull(ntee)
...
observe({
  ...
  which.mg <- ntee.values %>%
    dplyr::filter(nchar(ntee) == 1) %>%
    dplyr::filter(ntee %in% which.letter$MajorGroup)  %>%
    dplyr::select(description)
...
observe({
  ...
  which.tens <- ntee.values %>%
    ...
    dplyr::filter(substr(ntee, 1, 1) %in% which.letter$MajorGroup)  %>%
    dplyr::select(description)
